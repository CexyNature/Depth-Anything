"""
This script calculates the length of objects in images using depth maps. It processes images and their corresponding 
depth maps to compute the length in millimeters based on metadata provided in a CSV file. The script reads the image 
and depth map files, extracts coordinates and other relevant information from the metadata, and performs the necessary 
calculations. The results are then saved to a new CSV file.

Functions:
    swap_y_if_inverted(y0, y1, inverted): Swaps y-coordinates if the line is inverted.
    calculate_length(dmap, point1, point2, focal_length, zmin, zmax): Calculates the length between two points 
        in the depth map.
    process_image(input_image_path, output_depth_map_path, row, results): Processes an individual image and its 
        depth map, calculates the object length, and appends the result to the results list.
    main(input_dir, output_dir, csv_file_path): Main function that iterates through the metadata, processes each image, 
        and saves the results to a CSV file.

Usage:
    Run the script from the command line with the required arguments for input directory, output directory, and 
    CSV file path:
    
    python script.py -i <input_dir> -o <output_dir> -csv <csv_file_path>
"""
...
def calculate_length(dmap, point1, point2, focal_length, zmin, zmax):
    ...
    return length_metric * 1000  # Convert to millimeters
...
def process_image(input_image_path, output_depth_map_path, row, results):
    ...
    zmin = row[
        f"z-min-{MODEL}-{CAMERA}-10"
    ]  # Change column name according to model used
    zmax = row[
        f"z-max-{MODEL}-{CAMERA}-10"
    ]  # Change column name according to model used
    ...
    try:
        length_mm = calculate_length(dmap, point1, point2, FL, zmin, zmax)
        ...
        results.append(
            {
                ...
                f"zmin-{MODEL}-{CAMERA}-10": zmin,  # Change column name according to model used
                f"zmax-{MODEL}-{CAMERA}-10": zmax,  # Change column name according to model used
...
def main(input_dir, output_dir, csv_file_path):
                ...
                results.append(
                    {
                        ...
                        f"zmin-{MODEL}-{CAMERA}-10": row[
                            f"z-min-{MODEL}-{CAMERA}-10"
                        ],  # Change column name according to model used
                        f"zmax-{MODEL}-{CAMERA}-10": row[
                            f"z-max-{MODEL}-{CAMERA}-10"
                        ],  # Change column name according to model used
            ...
            results.append(
                {
                    ...
                    f"predicted_length_{MODEL}_{CAMERA}": "N/A",
                    f"focal_length_{CAMERA}": row[f"focal-length-{CAMERA}"],
                    f"zmin-{MODEL}-{CAMERA}-10": "N/A",  # Change column name according to model used
                    f"zmax-{MODEL}-{CAMERA}-10": "N/A",  # Change column name according to model used
                    "inverted_line": "N/A",
                }
            )
    ...
    results_df.to_csv(
        f"{SIZE}_predicted_length_10_line_{MODEL}_{CAMERA}.csv", index=False
    )
    print(f"Results saved to {SIZE} predicted_length_10_line_{MODEL}_{CAMERA}.csv")
